...
{  
	...
	/*连接流*/
	printf("MARK::1\n");
	m_pRtmp->Link.SWFSize = 0;
	if (RTMP_ConnectStream(m_pRtmp,0) == FALSE)    //报错了
	{
		RTMP_Close(m_pRtmp);
		RTMP_Free(m_pRtmp);
		return false;
...
int RTMPAAC_Send(void)
{
    ...
    fpIn = fopen("old.pcm", "rb");
	if(NULL == fpIn)
	{
		printf("fopen filename.pcm failed .....\n");
		return -1;
	}
    printf("start encode aac\n");
    // (1) Open FAAC engine
    ...
    printf("start encode 123\n");
    nPCMBufferSize = nInputSamples * nPCMBitSize / 8;  //1024*16/8=2048
	//printf("nInputSamples: %d  nPCMBitSize: %d\n ", nInputSamples, nPCMBitSize);
	...
	printf("aacObjectType: %lu\n", pConfiguration->aacObjectType);
	printf("outputFormat: %d\n", pConfiguration->outputFormat);
	//printf("");
   
    pConfiguration->inputFormat = FAAC_INPUT_16BIT;
	printf("start encode 234\n");
	...
	cap_rtmp_sendaac_spec(buf, len);  //发送aac头
    free(buf);
	printf("start encode 345\n");
    for(i = 0; 1; i++)
    {
        ...
        //if(nRet > 0)
        	cap_dialog_send_audio(pbAACBuffer, nRet) ; 
        printf("%d: faacEncEncode returns %d\n", i, nRet);
        //msleep(23);
        //memset(pbAACBuffer, 0, nRet);
        if(nBytesRead <= 0)
        {
            break;
        }
